...
- `npm test`：2026-09-02 在 macOS 合并验证为 498/498 通过；不同 shell 或平台发现数量可能不同，交付门槛始终是 0 fail。
...
- 事实越界、来源错位、评论污染和图片权利问题继续硬阻断；篇幅、解释深度和图片数量不足改为警告并保留草稿。
...
不要在服务运行时手工解压或覆盖 `.workflow`，也不要为了迁移关闭路径、指纹、权限或后台任务检查。
...
> 先完整阅读 AGENTS.md、README.md、docs/mature-personal-product.md 和 docs/WINDOWS-DEVELOPMENT-HANDOFF.md。运行 npm test、npm run eval:editorial、npm run build，确认 0 fail。不要改写产品方向，不要触碰或提交 .workflow。P0 跨系统完整归档迁移已经完成，默认进入 P1：使用真实来源连续制作 12 篇公众号草稿，记录选题到草稿耗时、终稿删除内容、图片使用率和阻断原因；发现文章不通顺时优先修素材包、稿型和事实主干，不继续叠加去 AI 味提示词。每次使用 Git 命令前，用中文说明命令作用和预期结果。
