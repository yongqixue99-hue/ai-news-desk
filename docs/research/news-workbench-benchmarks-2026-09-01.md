...
**2026-09-02 实施状态：** 上述 P0 已形成闭环。生成器现在逐段返回 `paragraphFactIds` 并校验事实与来源；完整度报告列出覆盖比例、未使用事实和缺失维度；纯内容警告保留草稿，同一修订最多运行一次只使用冻结包内未覆盖事实的精确补丁；应用后重新绑定来源并重跑质量门。黄金集增加“短而完整”与“长而欠覆盖”反例，目前为 22/22。
