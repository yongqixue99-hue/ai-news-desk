...
test("confirmed retrieval is stable, explainable, idempotent and removable without old facts entering prompts",async()=>{
 ...
 const plan=writingPreferencePlan(db,{title:"本地推理性能",intent:"news"});assert.equal(plan.method,"fts5-bm25");assert.equal(plan.selected[0]?.kind,"remove-promotional-language");assert.ok(plan.selected[0]!.evidence.every(e=>e.confirmationId));assert.doesNotMatch(JSON.stringify(plan, (key, value) => key === "eventId" ? undefined : value),/999/u);
