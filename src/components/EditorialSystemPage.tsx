import { WritingPreferencePreview } from "./WritingPreferencePreview";
  ...
  return (
    <div className="page editorial-system-page strategy-page">
      ...
      <div className="strategy-layout">
        ...
        <aside className="strategy-sidebar">
          ...
          <section className="strategy-section" aria-labelledby="automatic-reading-title">
            ...
            <div className="strategy-control-row"><div><strong>官方与热点增量检查</strong><p className="strategy-footnote">官方更新与新闻聚合源按所选间隔检查，技术目录每 12 小时读取。只收录候选，打开文章时再补正文与原图。</p></div>{toggle("officialMonitorEnabled", "官方与热点增量检查")}</div><div className="strategy-schedule"><label htmlFor="official-poll-interval">检查间隔</label><select id="official-poll-interval" value={settings.officialMonitorIntervalMinutes ?? 60} disabled={!settings.officialMonitorEnabled || settingBusy} onChange={event => void changeSettings({ officialMonitorIntervalMinutes: Number(event.target.value) })}><option value={30}>30 分钟</option><option value={60}>1 小时</option><option value={120}>2 小时</option><option value={240}>4 小时</option></select></div><p className="strategy-footnote">仅检查已启用且已选中的官方与发现来源（含 AIHOT）；连续失败会延长间隔。最近触发：{formatTime(settings.lastOfficialPollAt)}</p>
      ...
      <section className="strategy-records" aria-label="学习与运行记录">
        ...
        <WritingPreferencePreview version={JSON.stringify([memoryEnabled,view.writingMemories])} />
        <details className="strategy-details"><summary>查看记忆与依据 <span>{view.writingMemories.memories.length} 条写作偏好</span></summary><div className="strategy-details-body">
          <div className="writing-memory-list">{view.writingMemories.memories.map((memory) => <article key={memory.id} className={memory.enabled ? "" : "disabled"}><div><strong>{memory.label}</strong><span>{memory.evidenceCount} 次依据 · {!memoryEnabled || !memory.enabled ? "已暂停" : memory.applicable ? "可按稿件应用" : "等待稳定样本"}</span>{memory.evidence[0] ? <small>{memory.evidence[0].summary}</small> : null}<details><summary>查看全部保留依据</summary>{memory.evidence.map(evidence=><div className="memory-evidence" key={evidence.eventId}><p>{evidence.summary}</p><small>稿件 {evidence.draftId} · {evidence.createdAt} · {evidence.confirmationId ? "人工确认" : "旧记录，仅展示"}</small>{evidence.context?<p>选题：{evidence.context}</p>:null}{evidence.beforeExcerpt?<p>修改前：{evidence.beforeExcerpt}</p>:null}{evidence.afterExcerpt?<p>确认后：{evidence.afterExcerpt}</p>:null}</div>)}</details></div><div className="writing-memory-actions"><button type="button" disabled={memoryBusy === memory.id} onClick={() => void toggleMemory(memory.id, !memory.enabled)}>{memory.enabled ? "关闭" : "启用"}</button><button type="button" aria-label={"删除" + memory.label} disabled={memoryBusy === memory.id} onClick={() => void deleteMemory(memory.id, memory.label)}><Trash2 size={12} /></button></div></article>)}</div>
