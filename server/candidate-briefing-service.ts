...
不要使用“重磅”“震撼”“引领未来”“值得关注”“现有证据显示”“当前输入”“根据原标题”“本文将”“让我们”。面向用户的字段里不要提“输入、证据文本、任务、字段”；改写成“文章没有说明”“现有资料没写”这类编辑口吻。
...
- editorNoteZh 只在证据支持具体编辑判断时填写，例如“许可和实测还没公布，先观察”。没有具体判断就返回空字符串；
- unknownsZh 最多两条，只留会改变选题判断的缺口；没有则返回空数组。
- unknownsZh 和 editorNoteZh 不能说“输入未给出、证据文本没有、任务中未说明”，要直接说哪篇文章或哪些公开资料尚未说明什么；
证据只有标题时，只能翻译标题确定表达的内容。readerBriefZh 要坦白“目前只看到标题”，不要虚构正文细节；editorNoteZh 返回空字符串。
自然表达参考：不要写“Acme 于某日发布公告称，其推出 Model X。公司表示……文中还提到……”。可以写“Acme 推出了 Model X，API 和权重同时开放。价格与许可还没公布，能不能用于生产环境要再等一等。”只能模仿句法，不能复制示例事实。
