...
import { ArrowRight, Bookmark, Check, ExternalLink, Plus, RefreshCw, Search } from "lucide-react";
import { api } from "../api";
...
export function TopicCategoryTabs({ value, columns = defaultHomeLayout.columns, onChange, onCustomize }: { value: string; columns?: HomeColumn[]; onChange: (value: string) => void; onCustomize: () => void }) {
  return <div className="topic-category-bar"><div className="topic-category-tabs" role="tablist" aria-label="选题分类">
    {columns.map((category, index) => <button key={category.id} type="button" role="tab" id={`topic-tab-${category.id}`}
      ...
      onClick={() => onChange(category.id)} onKeyDown={(event) => {
        const next = event.key === "ArrowRight" ? (index + 1) % columns.length : event.key === "ArrowLeft" ? (index + columns.length - 1) % columns.length
          : event.key === "Home" ? 0 : event.key === "End" ? columns.length - 1 : undefined;
        if (next === undefined) return;
        event.preventDefault(); onChange(columns[next].id);
        document.getElementById(`topic-tab-${columns[next].id}`)?.focus();
      }}>{category.label}</button>)}
  </div><button type="button" className="topic-customize-button" aria-label="自定义栏目" title="添加、移除或排列栏目" onClick={onCustomize}><Plus size={20} /></button></div>;
}
...
const timeLabel = (value?: string) => value && Number.isFinite(Date.parse(value))
  ? new Date(value).toLocaleString("zh-CN", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "尚无读取记录";
...
export function TopicCategoryPanel({ platform, columnId = platform, keyword, onOpenStory, onChanged, onNavigate, onNotice }: Props) {
  ...
  return <section className="topic-category-panel" role="tabpanel" id={`topic-panel-${columnId}`} aria-labelledby={`topic-tab-${columnId}`} tabIndex={0}>
    ...
    {!loading && !items.length ? <div className="topic-feed-empty"><h3>{feed?.items.length ? "没有匹配的选题" : platform === "zhihu" ? "先读取一份真实热榜" : "还没有这个来源的选题"}</h3>
      <p>{feed?.items.length ? keyword ? "可以调整临时筛选，或在右上方 ＋ 中修改栏目关键词。" : "可以清除筛选，查看本次保存的全部内容。" : platform === "zhihu" ? "连接成功后，问题会出现在这里。" : "可在新闻工作台选择该来源采集，再回到这里浏览。"}</p>
    ...
    <div className="topic-feed-list" aria-busy={loading}>
      {items.map((item) => <article className="topic-feed-row" key={item.id}>
        ...
        <div><div className="topic-row-meta"><span>{item.rank ? `原榜 ${item.rank}` : names[platform]}</span>{item.metric ? <span>{item.metric}</span> : null}</div>
          <h3>{item.runId && item.candidateId ? <button type="button" disabled={Boolean(busy)} onClick={() => void act(item, "open")}>{item.title}</button> : <a href={item.url} target="_blank" rel="noreferrer">{item.title}</a>}</h3>
          ...
          <div className="topic-row-actions"><a href={item.discussionUrl || item.url} target="_blank" rel="noreferrer">{platform === "github" ? "查看项目" : "查看原帖"}<ExternalLink size={13} /></a>
